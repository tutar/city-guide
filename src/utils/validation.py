    ...
    def validate_external_url(url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            government_domains = [
                ...
                'immd.gov.hk', 'dsedt.gov.mo', 'gov.hk', 'gov.mo'
            ]

            is_government_url = any(domain in parsed.hostname.lower() for domain in government_domains)

            # Check for common appointment system patterns
            appointment_keywords = ['appointment', 'booking', 'reservation', 'schedule', 'apply']
            has_appointment_pattern = any(keyword in url.lower() for keyword in appointment_keywords)

            # Security checks
            security_issues = []
            if parsed.scheme == 'http':
                security_issues.append("HTTP protocol is not secure - consider using HTTPS")
            ...
            for pattern in suspicious_patterns:
                if re.search(pattern, url):
                    security_issues.append(f"URL contains suspicious pattern: {pattern}")
    ...
    def validate_appointment_url(url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            appointment_indicators = {
                "secure_form": any(pattern in url.lower() for pattern in ['form', 'apply', 'submit']),
                "calendar": any(pattern in url.lower() for pattern in ['calendar', 'schedule', 'timeslot']),
                "confirmation": any(pattern in url.lower() for pattern in ['confirm', 'success', 'thank']),
                "login_required": any(pattern in url.lower() for pattern in ['login', 'signin', 'auth']),
                "appointment": any(pattern in url.lower() for pattern in ['appointment', 'booking', 'reservation'])
            }

            # Check for required appointment features
            has_appointment_features = (
                appointment_indicators["secure_form"] or
                appointment_indicators["calendar"] or
                appointment_indicators["confirmation"] or
                appointment_indicators["appointment"]
            )
        ...
        try:
            ...
            if 'label' in option:
                ...
                if not label or not label.strip():
                    ...
                    warnings.append("Label is very long - consider shortening for better user experience")
            ...
            if 'action_type' in option and option['action_type'] not in valid_action_types:
                issues.append(f"Invalid action_type: {option['action_type']}. Must be one of: {valid_action_types}")
            ...
            if 'priority' in option:
                ...
                if not isinstance(priority, int):
                    ...
                    warnings.append("Priority should be between 1 and 10 (1 = highest priority)")
        ...
        try:
            ...
            if 'description' in category and category['description']:
                ...
                if len(description) > 1000:
                    warnings.append("Description is very long - consider summarizing for better user experience")

            # Validate official source URL
            if 'official_source_url' in category and category['official_source_url']:
                url_validation = URLValidator.validate_external_url(category['official_source_url'])
                if not url_validation["valid"]:
                    issues.append(f"Invalid official source URL: {url_validation['error']}")
                elif not url_validation["is_government_url"]:
                    warnings.append("Official source URL is not from a recognized government domain")
...
def validate_and_sanitize_input(input_text: str, max_length: int = 1000) -> Dict[str, Any]:
