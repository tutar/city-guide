...
class TestPassportGuidance:
    ...
    def mock_search_service(self):
        ...
        mock.generate_dynamic_navigation_options.return_value = [
            {"label": "Material Requirements", "action_type": "requirements", "priority": 1},
            {"label": "Online Appointment", "action_type": "appointment", "priority": 2},
    ...
    @pytest.mark.asyncio
    async def test_passport_guidance_conversation_start(self, mock_ai_service, mock_search_service, passport_service_category):
        ...
        with patch('src.services.ai_service.AIService', return_value=mock_ai_service), \
             patch('src.services.search_service.SearchService', return_value=mock_search_service):
            ...
            navigation_options = await mock_search_service.generate_dynamic_navigation_options(
            ...
            assert any("Material Requirements" in option["label"] for option in navigation_options)
    ...
    @pytest.mark.asyncio
    async def test_material_requirements_navigation(self, mock_search_service, passport_service_category):
        ...
        material_option = next(
            (option for option in navigation_options
    ...
    async def test_conversation_context_persistence(self):
        ...
        assert conversation.conversation_history[2].content == "What documents do I need?"
    ...
    @pytest.mark.asyncio
    async def test_unanswerable_query_handling(self, mock_ai_service, mock_search_service):
