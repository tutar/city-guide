...
from src.models.embeddings import SearchResult, HybridSearchRequest
...
class TestSearchServiceNavigation:
    ...
    def setup_method(self):
        ...
        with patch('src.services.search_service.EmbeddingService', return_value=self.mock_embedding_service), \
             patch('src.services.search_service.AIService', return_value=self.mock_ai_service), \
             patch('src.services.search_service.BM25Service', return_value=self.mock_bm25_service):
            self.search_service = SearchService()

    @patch('src.services.search_service.DataService')
    @patch('src.services.search_service.AIService')
    def test_generate_dynamic_navigation_options_with_search_results(self, mock_ai_service, mock_data_service):
        ...
        assert navigation_options[0]['label'] == "Learn more about Hong Kong Passport Requirements"
        assert navigation_options[0]['action_type'] == "explain"
        assert navigation_options[0]['target_url'] == "https://immd.gov.hk/passport"

    @patch('src.services.search_service.DataService')
    @patch('src.services.search_service.AIService')
    def test_generate_navigation_options_with_service_category(self, mock_ai_service, mock_data_service):
        ...
        mock_data_service.return_value.__enter__.return_value = mock_data_service_instance
        ...
        mock_data_service_instance.get_navigation_options_by_category.return_value = mock_navigation_options
    ...
    def test_generate_navigation_options_with_ai_suggestions(self):
        ...
        mock_ai_suggestions = [
            ...
            {
                "label": "Find Renewal Centers",
                "action_type": "navigate",
                "priority": 3
            }
        ]
        self.mock_ai_service.generate_navigation_suggestions.return_value = mock_ai_suggestions
    ...
    @patch('src.services.search_service.DataService')
    @patch('src.services.search_service.AIService')
    def test_empty_navigation_options_on_error(self, mock_ai_service, mock_data_service):
        ...
        mock_data_service_instance = Mock()
        mock_data_service.return_value.__enter__.return_value = mock_data_service_instance
        mock_data_service_instance.get_navigation_options_by_category.side_effect = Exception("Database error")

        # Mock AI service to raise exception
        mock_ai_service.return_value.generate_navigation_suggestions.side_effect = Exception("AI error")
